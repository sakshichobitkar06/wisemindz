...
export default (requestData, id = 0) => {
    const schema = Joi.object({
        ...
        school_id: Joi.number().min(1).messages({
            'number.base' : `school id should be a type of 'number'`,
            'number.empty': `school id cannot be an empty field`,
            'number.min'  : `school id length must be at least 1 characters`,
        ...
        designation_id: Joi.number().min(1).required().messages({
            'number.base' : `designation id should be a type of 'number'`,
            'number.empty': `designation id cannot be an empty field`,
            'number.min'  : `designation id length must be at least 1 characters`,
            'any.required': `designation id is a required field`
        })
    });
